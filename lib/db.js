class db {
    ...
    itemInsertVariant(name, name_hash, tags) {
        return new Promise(async (resolve, reject) => {
            await this.pool.query(`INSERT INTO ${this.cfg.table.item_variants} (name, name_hash, tags) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id);`, [name, name_hash, JSON.stringify(tags)])
                .then((m) => {
                    return resolve(m[0].insertId);
                }).catch((e) => {
                    console.log(e.errno + ": " + e.sqlMessage);
