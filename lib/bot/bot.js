...
class Bot extends EventEmitter {
    ...
    async queueTask(queue) {
        ...
        switch (queue.type) {
            ...
            case 2: // inspect (float, pattern)
                response = await this.itemInspect(queue.params)
                    .then(e => e)
                    .catch(e => { return e; });
                break;
        }
    ...
    itemInspect(params) {
        return new Promise(async (resolve, reject) => {
            if (typeof (params.a) === 'undefined' || typeof (params.d) === 'undefined') return reject({ err: { message: "no param", id: -1 } });
            //
            let sid = "";
            if (typeof (params.usid) !== 'undefined') {
                let inspected = await this.controller.base.db.itemInspectGetByUser(params.usid, params.a).catch(e => { return null });
                if (inspected !== null) {
                    if (inspected.length > 0) return reject({ insectID: inspected[0].id });
                }
                //
                ...
                sid = user.sid;
            } else if (typeof (params.bid) !== 'undefined') {
                let inspected = await this.controller.base.db.itemInspectGetByItem(params.item_id, params.a).catch(e => { return null });
                if (inspected !== null) {
                    if (inspected.length > 0) return reject({ insectID: inspected[0].id });
                }
                //
                let tbbot = await this.controller.base.db.tbbotGet(params.bid).catch(e => { return null });
                if (tbbot === null) return reject({ err: { message: "no tbbot", id: -1 } });
                if (tbbot.sid === null) return reject({ err: { message: "no steamid", id: -1 } });
                sid = tbbot.sid;
            } else if (typeof (params.s) !== 'undefined') {
                sid = params.s;
            }
            else {
                return reject({ err: { message: "no sid param", id: -1 } });
            }
            //
            let itemData = null;
            let received = false;
            ...
            this.log(`${sid}, ${params.a}, ${params.d}`, `inspect`, `user#${params.usid}`)
            this.game.inspectItem(sid, params.a, params.d, (data) => {
                itemData = data;
                received = true;
            });
            //
            ...
            if (received) {
                ...
                let delay = this.controller.base.cfg.time.inspectDelay - offset;
                if (delay < 0) delay = 0;
                //
                ...
                // bb.readDoubleBE(0);
                // js
                // let num = BigInt("4595667872935575552");
                ...
                //
                let db_insert = null;
                if (typeof (params.iv_id) !== 'undefined') {
                    // if has phase
                    let phase = this.controller.getDopplerPhase(itemData.paintindex);
                    if (phase) {
                        let itemVariant = await this.controller.base.db.itemGetVariant(params.iv_id).catch(e => {
                            return reject({ err: { message: "can't add row in db#itemVariant", original: e.message, id: -1 }, delay: delay });
                        });
                        if (itemVariant === null) reject({ err: { message: "can't add row in db#itemVariant", id: -1 }, delay: delay });
                        let itemWear = this.controller.itemNameGetWear(itemVariant.name_hash);
                        let itemName = itemVariant.name_hash;
                        if (itemWear) itemName = itemName.replace(itemWear[0], '').trim();
                        itemName += ` ${phase}`;
                        if (itemWear) itemName += ` ${itemWear[0]}`;
                        //
                        let itemVariantPhase = await this.controller.base.db.itemGetVariantByNameHash(itemName).catch(e => {
                            return reject({ err: { message: "can't get row in db#itemVariantPhase", original: e.message, id: -1 }, delay: delay });
                        });
                        if (itemVariantPhase === null) {
                            let item_variant_db_id = await this.controller.base.db.itemInsertVariant(itemVariant.name, itemName, itemVariant.classid, itemVariant.tags).catch((e) => { return null });
                            if (item_variant_db_id === null) { this.log('db error on itemInsertVariant', 'inventoryItemsScan'); } // !
                            params.iv_id = item_variant_db_id;
                        } else {
                            params.iv_id = itemVariantPhase.id;
                        }
                    }
                }
                if (typeof (params.usid) !== 'undefined') {
                    db_insert = await this.controller.base.db.itemInspectInsert(params.usid, params.a, params.d, itemData, params.iv_id).catch((e) => { return null; });
                } else if (typeof (params.bid) !== 'undefined') {
                    db_insert = await this.controller.base.db.itemInspectInsert(null, params.a, params.d, itemData, params.iv_id, params.item_id).catch((e) => { return null; });
                }
    ...
    userInventoryFromTrade(params) {
        return new Promise(async (resolve, reject) => {
            ...
            await this.inventoryItemsScan(them_inventory).then(async (data) => {
                await this.controller.base.db.inventoryInsert(params.usid, data)
                    .catch((e) => {
    ...
    inventoryItemsScan(inventory) {
        return new Promise(async (resolve, reject) => {
            ...
            for (let item of inventory) {
                ...
                //
                let item_variant_db_id = await this.controller.base.db.itemInsertVariant(item["name"], item["market_hash_name"], item["classid"], item_tags).catch((e) => { return null });
