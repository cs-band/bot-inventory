...
class Bot extends EventEmitter {
    ...
    async queueTask(queue) {
        ...
        switch (queue.type) {
            case 1: // inventory
                queue.response = await this.userInventoryFromTrade(queue.params)
                    .then(e => { return e; })
                    .catch(e => { return e; });
                break;
            case 2: // inspect (float, pattern)
                queue.response = { success: false };
                break;
        }
        //
        queue.done = true;
    ...
    async userInventoryFromTrade(params) {
    ...
    inventoryItemsScan(inventory) {
        return new Promise(async (resolve, reject) => {
            ...
            for (let item of inventory) {
                ...
                //
                let item_variant_db_id = await this.controller.base.db.itemInsertVariant(item["name"], item["market_hash_name"]).catch((e) => { return null });
                ...
                let item_args = this.getItemArgs(item);
                //
                item_args["item_variant"] = item_variant_db_id;
                ...
                if (item["name"].indexOf("StatTrak™") > -1) {
                    ...
                    if (stIndex > -1) item_args["stattrak_kills"] = parseInt(/(\d+)/.exec(item["descriptions"][stIndex].value)[1]);
                    else item_args["stattrak_kills"] = 0;
